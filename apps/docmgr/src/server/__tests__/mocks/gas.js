    ...
    this._sheets[name] = {
      ...
      getRange(row, col, numRows, numCols) {
        ...
        return {
          ...
          getValues() {
            const out = []
            for (let r = 0; r < (numRows || 1); r++) out.push(sheet._rows[row - 1 + r].slice(col - 1, col - 1 + (numCols || 1)))
            return out
          },
          setValues(vals) {
      ...
      setName(n) {
        if (SpreadsheetApp._sheets[this._name] === this) {
          delete SpreadsheetApp._sheets[this._name]
          SpreadsheetApp._sheets[n] = this
        }
        this._name = n
        return this
      },

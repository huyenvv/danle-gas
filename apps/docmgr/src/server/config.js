...
function ensureInitialized() {
  ...
  if (props.getProperty('SCHEMA_V') === '14') {
    // File đã ở schema 14 → vẫn cần backfill 1 lần (idempotent qua cờ riêng).
  ...
  // 012: cột Hồ Sơ vừa có thể thay đổi → xoá cache map {tên cột → chữ cái} để đọc lại header.
  if (typeof _invalidateDocColsCache === 'function') _invalidateDocColsCache()

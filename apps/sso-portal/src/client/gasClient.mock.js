// Dev mock — in-memory stand-in for the GAS backend.
// Excluded from the production bundle: gasClient.js only dynamic-imports this
// when process.env.NODE_ENV !== 'production' (dead-code-eliminated by Vite/Rollup).
...
const _mockUsers = [
  { ID: 1,  'Tên đăng nhập': 'admin@test.com', 'Email': 'admin@test.com',      'Tên nhân viên': 'Admin Hệ thống',    'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '2024-01-15', 'FailedLogins': 0 },
  { ID: 2,  'Tên đăng nhập': 'huyenvv',        'Email': 'huyenvv.it@gmail.com', 'Tên nhân viên': 'Vũ Văn Huyên',      'Trạng thái': 'Active', 'MustChangePass': 'TRUE',  'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 3,  'Tên đăng nhập': 'nhanvien1',      'Email': 'nv1@test.com',         'Tên nhân viên': 'Nguyễn Văn A',      'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 4,  'Tên đăng nhập': 'giamdoc',        'Email': 'gd@test.com',          'Tên nhân viên': 'Lê Văn Giám Đốc',  'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 5,  'Tên đăng nhập': 'phogd1',         'Email': 'pgd1@test.com',        'Tên nhân viên': 'Phạm Phó GĐ 1',    'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 6,  'Tên đăng nhập': 'phogd2',         'Email': 'pgd2@test.com',        'Tên nhân viên': 'Hoàng Phó GĐ 2',   'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 7,  'Tên đăng nhập': 'vanthu',         'Email': 'vt@test.com',          'Tên nhân viên': 'Ngô Thị Văn Thư',  'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 8,  'Tên đăng nhập': 'nhanvien2',      'Email': 'nv2@test.com',         'Tên nhân viên': 'Đỗ Minh Tuấn',     'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 9,  'Tên đăng nhập': 'tpkd',           'Email': 'tpkd@test.com',        'Tên nhân viên': 'Vũ Thị Hương',     'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 10, 'Tên đăng nhập': 'nvkd1',          'Email': 'nvkd1@test.com',       'Tên nhân viên': 'Bùi Đức Thắng',    'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
  { ID: 11, 'Tên đăng nhập': 'newuser',        'Email': 'new@test.com',         'Tên nhân viên': 'Người Mới',         'Trạng thái': 'Active', 'MustChangePass': 'FALSE', 'Đăng nhập cuối': '',           'FailedLogins': 0 },
]

const _mockPhongBan = [
  { ID: 1, 'Tên phòng ban': 'Kỹ thuật', 'Mô tả': 'Phòng kỹ thuật & phát triển', 'Đơn vị thuộc sự quản lý': '' },
  { ID: 2, 'Tên phòng ban': 'Kinh doanh', 'Mô tả': 'Phòng kinh doanh & marketing', 'Đơn vị thuộc sự quản lý': '' },
]
...
export async function mockCall(fn, ...args) {
  ...
  switch (fn) {
    case 'api_login': {
      ...
      const hasAdminPos = _mockAssignments.some(a => String(a['UserID']) === String(mockUser.ID) && a['Chức vụ'] === 'admin')
      const isAdmin = isOwner || hasAdminPos
    ...
    case 'api_getUsers': {
      ...
      return _mockUsers.filter(u => {
        if (u['Email'] === 'admin@test.com') return false
        if (isOwner) return true
        return !_mockAssignments.some(a => String(a['UserID']) === String(u.ID) && a['Chức vụ'] === 'admin')
      }).map(u => ({ ...u }))
    }
    case 'api_addUser': {
      _pushLog('Thêm', 'Người dùng', args[1]['Email'] || '', '')
      const newUser = { ID: ++_nextId, ...args[1], 'Trạng thái': 'Active', 'MustChangePass': 'TRUE', 'FailedLogins': 0 }
    ...
    case 'api_addPhongBan': {
      const newDept = { ID: ++_nextId, 'Tên phòng ban': args[1]['Tên phòng ban'], 'Mô tả': args[1]['Mô tả'] || '', 'Đơn vị thuộc sự quản lý': args[1]['Đơn vị thuộc sự quản lý'] || '' }
    ...
    case 'api_updatePhongBan': {
      ...
      if (dept && deptUpdates) {
        for (const k of ['Tên phòng ban', 'Mô tả', 'Đơn vị thuộc sự quản lý']) {

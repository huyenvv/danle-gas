...
## Cập nhật mô hình v2 — LINK thay vì COPY (2026-06-10)

Sau phản hồi thực tế: luồng "chọn từ Drive" khi tạo document chuyển từ **copy** sang **link** (trỏ tới file đã có sẵn trên Drive). Mục đích: liên kết document với file đã được tổ chức sẵn theo cây thư mục, **tránh nhân bản / sai folder**. (Upload từ máy **giữ nguyên** mô hình copy như cũ.)

### Hành vi mới (đã chốt)
- **Link, không copy:** document gắn thẳng `fileId` của file gốc; không tạo bản sao.
- **Suy danh mục từ vị trí file:** danh mục của document được xác định theo **folder chứa file**, khớp **đúng path tên** với cây danh mục trong app (`ROOT_FOLDER_ID/<tên danh mục cha>/<tên danh mục con>`).
  - Đã chọn danh mục ở form mà **lệch** với folder của file → **báo lỗi**.
  - Chưa chọn → **tự điền** danh mục theo folder của file.
  - Folder của file **không khớp** danh mục nào trong app → **báo lỗi**: cần tạo danh mục tương ứng ở app trước.
- **Chỉ trong cây app:** picker **khóa trong `ROOT_FOLDER_ID`**, không chọn được file ngoài cây (bỏ lối thoát "My Drive" ở luồng document).
- **Nhiều file khác danh mục** (ở các folder khác nhau) → **báo lỗi** (1 document chỉ 1 danh mục).
- **Trộn nguồn** (link-Drive + upload-máy trong cùng document) → khóa thống nhất 1 danh mục.
- **Không xóa file link:** gỡ đính kèm / xóa document **không** trash file gốc trên Drive (chỉ bỏ liên kết). `fileInfo.linked = true` đánh dấu để bỏ qua mọi điểm trash.
- **Chia sẻ:** nếu file gốc chưa share → set `ANYONE_WITH_LINK / VIEW` khi link.

### Thay thế/đụng tới phần đã build
- `api_copyDriveFiles` / `copyDriveFilesToCategory` (makeCopy) → thay bằng `api_linkDriveFiles` / `_resolveCategoryForFile` (validate + link, không copy).
- `DriveFilePicker`: thêm chế độ khóa trong app root (không có crumb My Drive) cho luồng document. Import Excel **không đổi**.

### Chính sách xóa file trên Drive (v2)
Helper `_shouldTrashFile(fileInfo, docStatus)` = `!fileInfo.linked && docStatus === 'Nháp'`. Áp tại mọi điểm xóa:
- **File link Drive:** không bao giờ trash (chỉ bỏ liên kết).
- **File upload từ máy:** chỉ trash khi document đang ở **Nháp** (`deleteDocument`, `updateDocument` gỡ file, `cancelDraft`).
- **Xóa document ở trạng thái khác Nháp** (admin hay ai): **không** trash file nào → để **orphaned**, tool dọn rác xử lý sau.
- `deleteFiles` (gỡ file eager tạm trong modal): vẫn trash file máy; client **không** gọi cho file `linked`.
